...
public class PlayerDragging : NetworkBehaviour
{
    ...
    [SerializeField] private KeyCode dragKey = KeyCode.E;
    ...
    private void Drag()
    {
        if (!isOwned || draggedRigidBody == null) return;
        ...
        {
            if (draggedRigidBody != null)
                CmdAddForceToDraggedObject(draggedRigidBody.gameObject, (transform.position - draggedRigidBody.transform.position).normalized * (distance - dragRange) * 300 * Time.deltaTime);
